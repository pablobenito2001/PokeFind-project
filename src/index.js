...
(function(){
    ...
    let card = GetData.getDataOne(URL_POKEMON + nro.toString())
    .then(elem => document.getElementById('cardHeader').innerHTML = elem.renderCard('pokeHeader'))
    setInterval(()=>{
        nro = Math.floor(Math.random() * 905);
        card = GetData.getDataOne(URL_POKEMON + nro.toString())
        .then(elem => document.getElementById('cardHeader').innerHTML = elem.renderCard('pokeHeader'));
    }, 8000)
})();
